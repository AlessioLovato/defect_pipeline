...
class WorldPointCloudGeneratorNode(Node):
    ...
    def _generate_from_params(self) -> tuple[bool, str, int]:
        ...
        return self._generate(world_sdf_path, resolution, bbox_min, bbox_max, auto_save, pcd_path)

    def _generate(self, world_sdf_path: str, resolution: float, bbox_min: np.ndarray, bbox_max: np.ndarray,
        ...
        try:
            points = parse_world_to_pointcloud(world_sdf_path, resolution, bbox_min, bbox_max)
        ...
        return True, f'generated {self.current_count} points', self.current_count
